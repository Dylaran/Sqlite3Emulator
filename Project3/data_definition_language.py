...
Added the following functionalities:
    Changes to the connect function (database, timeout, isolation_level)
    Raising Exceptions
    Regression testing
    Connections testing
    Create_Drop_Table
...
Sources:
    Professor Joshua Nahum's project 3 starter code as a base
...
class Connection(object):
    ...
    def __init__(self, filename, timeout = 0.1, isolation_level = None):
        """
        ...
        # Default mode (Transactions that don't specify a mode default to DEFERRED)
        # Locks are acquired when needed by a statement in the transaction
        self.DEFERRED = "DEFERRED"
                                
        # A reserved lock is acquired at start of the transaction
        # and an exclusive lock is acquired when needed
        self.IMMEDIATE = "IMMEDIATE"
        
        # An exclusive lock is acquired at the start of transaction
        self.EXCLUSIVE = "EXCLUSIVE"
    
        # Boolean for transaction start
        self.start = False
        
        # Copy of database
        self.temp = Database(filename) 
        
        # Table locks
        self.table_locks = {}
        self.lock = len(locks.keys())
        locks[len(locks.keys())] = 0
        temp_locks[len(locks.keys()) - 1] = self.table_locks
    ...
    def execute(self, statement):
        ...
        def create_table(tokens):
            ...
            # Check 'IF NOT EXISTS' exists 
            # EX: CREATE TABLE IF NOT EXISTS students...
            if tokens[0] == "IF":
                pop_and_check(tokens, "IF")
                pop_and_check(tokens, "NOT")
                pop_and_check(tokens, "EXISTS")
                
                curr_tables = _ALL_DATABASES[self.file].tables
                if tokens[0] in curr_tables:
                    return
        ...
        def insert(tokens):
            ...
            while tokens:
                ...
                for key in locks.keys():
                        if self.lock != key:
                            if table_name in temp_locks[key].keys():
                                if temp_locks[key][table_name] in {2, 3}:
                                    raise Exception("Unable to insert")
                if self.start == True:
                    if temp_locks[self.lock][table_name] != 3:
                        temp_locks[self.lock][table_name] = 2
                        locks[self.lock] = 2
                        
                    self.temp.insert_into(table_name,
                                          row_contents,
        ...
        def update(tokens):
            ...
            if self.start == True:
                if temp_locks[self.lock][table_name] != 3:
                    temp_locks[self.lock][table_name] = 2
                    locks[self.lock] = 2
                self.temp.update(table_name, update_clauses, where_clause)
        ...
        def delete(tokens):
            ...
            if self.start == True:
                if temp_locks[self.lock][table_name] != 3:
                    temp_locks[self.lock][table_name] = 2
                    locks[self.lock] = 2
                self.temp.delete(table_name, where_clause)
            else:
        ...
        def select(tokens):
            ...
            for key in locks.keys():
                    if self.lock != key:
                        if table_name in temp_locks[key].keys():
                            if temp_locks[key][table_name] == 3:
                                raise Exception
                            
            if self.start == True:
                if locks[self.lock] not in {2, 3}:
                    locks[self.lock] = 1
                    temp_locks[self.lock][table_name] = 1
                return self.temp.select(
                    output_columns,
                    order_by_columns,
        ...
        def drop_table(tokens):
            """
            Drops table if exists, raises exception if table doesn't exist 
            and "IF EXIST" does not exist
            """
            ...
            if tokens[0] == "IF":
                ...
                table_name = tokens.pop(0)
                curr_tables = _ALL_DATABASES[self.file].tables
                if table_name in curr_tables:
                    del _ALL_DATABASES[self.file].tables[table_name]
                else:
                    return
            else:
                table_name = tokens.pop(0)
        ...
        def begin_transaction(tokens):
            ...
            Raise an exception if a BEGIN TRANSACTION occurs within an open transaction.
            """
            ...
            # Check if other mode specified
            if tokens[0] in {self.DEFERRED, self.IMMEDIATE, self.EXCLUSIVE}:
                mode = tokens[0]
            ...
            if mode == self.DEFERRED:
                ...
                for table in self.database.tables:
                    self.table_locks[table] = 2
                    
            elif mode == self.EXCLUSIVE:
                for key in locks.keys():
                    if self.lock != key:
                        if locks[key] in {2, 3}:
                            self.start = False
                ...
                for table in self.database.tables:
                    self.table_locks[table] = 3
            else:
                raise Exception("Not possible")
                
            self.temp.tables = deepcopy(self.database.tables)
                
        def commit_transaction(tokens):
            """
            Transactions end with COMMIT TRANSACTION.
            Raise an exception, if a COMMIT TRANSACTION is attempted
            without a prior BEGIN TRANSACTION
            """
            if self.start == False:
                raise Exception("Transaction not open")
            else:
                self.start = False
            ...
            for key in locks.keys():
                if self.lock == key:
                    ...
                    for temp_key in temp_locks[key].keys():
                        if temp_locks[key][temp_key] == 1:
                            raise Exception
                            
            self.database.tables = deepcopy(self.temp.tables)
            locks[self.lock] = 0
            
            for lock in temp_locks[self.lock]:
                temp_locks[self.lock][lock] = 0
                
        def rollback_transaction(tokens):
            """
            The second way a transaction can end is with a rollback.
            All the changes made by the transaction needs to be reversed, and all locks released.
            
            You can only rollback within a manual transaction, if a connection tries to rollback
            outside of a transaction, raise an error
            """
...
def connect(filename, timeout = 0, isolation_level = None):
    ...
    """
    return Connection(filename, timeout = 0, isolation_level = None)
